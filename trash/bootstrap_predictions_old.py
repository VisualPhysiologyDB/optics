...
def calculate_ensemble_CI_parallel(model_folder, query, name, predictions_dict):
    def predict_with_model(filename):
        model_path = os.path.join(model_folder, filename)
        model = load_obj(model_path)
        prediction = model.predict(query)
        return round(float(prediction[0]), 1)

    filenames = [f for f in os.listdir(model_folder) if f.endswith('.pkl')]

    # Parallelize predictions
    predictions_all = Parallel(n_jobs=-1)(delayed(predict_with_model)(filename) for filename in filenames) 
