...
def model_compare_cv(X, y, preprocess_pipe, models_dict, scoring, cv,
    ...
    if isinstance(cv, (list)):
        cv = PredefinedSplit(test_fold=cv)  # test_fold indicates the fold each sample belongs to (-1 for samples not in a test fold)
    else:
    ...
    for model in top_models:
        ...
        if len(p_grid) > 0:
            ...
            try:
                ...
                # R-squared for all combinations
                #all_r_squared = grid_search.cv_results_['mean_test_score']
                #print("R-squared for each parameter combination:", all_r_squared)
            except:
                pass    
            ...
            print("No Grid-Search Specified, fitting model to default parameters.")
            model.fit(X=X, y=y)
            if report_dir is not None:
...
def aaprop_importance_from_pipe(model, n_positions=None, grouped_features=None):
    """
    Calculate feature importance from a pipeline containing a feature selection/preprocessing step and a model.
    ...
    # Get the number of features in the input dataset
    if n_positions is None:
        #n_positions = model.named_steps['prep']['aa_prop'].get_n_features_in()
        n_positions = model.named_steps['prep']['aa_prop'].get_n_features_in()
    
        #try:
            #n_positions = len(model[-1].feature_names_in_)
        #except:
            #n_positions = len(model[-1].feature_importances_)
    ...
    # Extract the feature indices from the column names
    #tmp['feature'] = tmp['feature'].str.split('_').str[0]
    tmp['feature'] = tmp['feature'].str.split('p').str[1]

    # Aggregate feature importance values for each feature index and normalize by the maximum value
    #tmp = tmp.groupby('feature')['value'].mean().reset_index()
    tmp2 = pd.DataFrame(range(1, n_positions), columns=['feature'])
    tmp2 = pd.concat([tmp2, tmp], axis = 0, ignore_index=True)
    tmp.fillna(0, inplace=True)
    tmp['standard_value'] = tmp['value'] / np.max(tmp['value'])

    return tmp.to_dict(orient='list')
...
def mean_importance(top_models: list, n_positions: int = None, grouped_features: dict = None, report_dir: str = None) -> pd.DataFrame:
    ...
    for n, model in enumerate(top_models):
        ...
        if 'aa_prop' in model.named_steps['prep'].named_steps:
            imp = aaprop_importance_from_pipe(model, n_positions=n_positions, grouped_features=grouped_features)
        else:
        ...
        if cor_mat is None:
            ...
            try:  
                if 'collinear_care' in top_models[0].named_steps['prep'].named_steps:
                    cor_mat = top_models[0].named_steps['prep']['collinear_care'].get_corr()
                    cor_mat.to_csv(str(report_dir + '/' + 'correlation' + '.csv'), index=True)
            except:
                try:
                    if 'collinear_care' in top_models[1].named_steps['prep'].named_steps:
                        corr_mat = top_models[1].named_steps['prep']['collinear_care'].get_corr()
                        corr_mat.to_csv(str(report_dir + '/' + 'correlation' + '.csv'), index=True)
                except:
                    print("Correlation Matrix Not Accessible - Passing Over This Request")
                    pass
    return p_values
