...
def fetch_protein_sequence(accession, sequence_type='target', db_order=None):
    """
    Fetches a protein sequence from NCBI databases or prompts manually.

    This function attempts to query the preferred NCBI database first (nucleotide
    or protein) and automatically falls back to the other if no appropriate 
    features or records are found.

    Args:
        accession (str): The accession name or number. Can also be 'manual'.
        sequence_type (str): Type of sequence ('reference' or 'target').
        db_order (list): Ordered list of databases to try.
    ...
    for db in db_order:
        try:
            ...
            print(f"  - Error fetching or parsing {accession} from {db}: {e}")
            continue # Try the next db fallback
...
def generate_dms_library(
    ...
    reference_protein = Protein(fetch_protein_sequence(reference_accession, 'reference', db_order))

    print(f"\nFetching wild-type sequence '{wt_accession}'...")
    wt_protein = Protein(fetch_protein_sequence(wt_accession, 'target', db_order))
    ...
    for site in sites:
        ...
        for new_aa in AMINO_ACIDS:
            if actual_aa == new_aa:
                ...
                wt_protein_str = str(wt_protein)
                mutant_seq_list = list(wt_protein_str)
