...
def fetch_sequence_from_ncbi(accession, db_order):
    """
    Fetches a protein sequence from a nucleotide accession, protein accession, or a predefined dict.

    This function iterates through the preferred database order.
    For nucleotide records, it parses the translated protein sequence from the 'CDS' feature.
    For protein records, it pulls the sequence directly.

    Args:
        accession (str): The accession name or number. Can also be 'manual'.
        db_order (list): List of databases to try, e.g., ['nucleotide', 'protein'].
    ...
    for db in db_order:
        try:
            print(f"Fetching '{accession}' from NCBI {db.capitalize()} DB...")
            ...
            if db == "nucleotide":
                ...
                # If no CDS with translation is found, raise error to trigger fallback
                raise ValueError(f"No CDS translation found for {accession} in {db}")
            
            elif db == "protein":
                # For protein DB, the sequence is the protein
                return str(record.seq)

        except Exception as e:
            print(f"  - Error fetching/parsing {accession} from {db}: {e}")
            continue # Try the next database in the fallback list
            
    # If all databases in the db_order fail:
...
def create_chimera(segments, point_mutations, reference_accession, db_order=['nucleotide','protein']):
    """
    ...
    Args:
        ...
        db_order (list): DB priority list to fetch from.

    Returns:
        str: The final amino acid sequence of the chimera.
    """
    print(f"\nFetching reference sequence '{reference_accession}'...")
    reference_protein = Protein(fetch_sequence_from_ncbi(reference_accession, db_order))
    substitution_matrix = substitution_matrices.load("BLOSUM62")
    chimera_pieces = []
    ...
    for seg in segments:
        ...
        wt_protein = Protein(fetch_sequence_from_ncbi(acc, db_order))
...
def main():
    ...
    parser.add_argument(
        ...
        default='NM_001014890', # Bos taurus rhodopsin DNA
    ...
    try:
        ...
        final_chimera_sequence = create_chimera(
            segments, point_mutations, args.reference_accession, db_order
        )
