...
class OpticsGUI(tk.Tk):
    def __init__(self, mode='predictions'):
        super().__init__()
        self.mode = mode
        self.wants_to_go_back = False # Flag to track navigation
...
def main():
    while True:
        ...
        # If the selector was closed without a choice (e.g. X button), break the loop
        if not mode:
            break
        ...
        # 4. Check navigation flag
        # If the user didn't click "Back" (e.g. they clicked X), break the loop
        if not getattr(app, 'wants_to_go_back', False):
            break
