...
def plot_predictions_with_CI(names, predictions, mean_preds, pdf_file):
    if os.path.exists(pdf_file):
        ...
        for i, seq_name in enumerate(concat_names):
            #x_pos = i + 1  # Center the text horizontally 
            #y_pos = bplot['medians'][i].get_ydata()[1] - 0.4  # Place slightly below the median
            #plt.text(x_pos, y_pos, seq_name, ha='center', va='top', color='black', fontsize=10, zorder = 3)  # Add text label
